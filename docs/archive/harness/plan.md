...
> 상태: 참고 자료. 단계별 적용 후보와 run artifact 계약 초안을 정리한 문서이며, active checklist나 필수 gate가 아니다.

## 이 문서가 답하는 것

- 코드나 실험을 시작하기 전, "지금 단계에서 어디까지 만들어야 하나"를 잡으려고 본다.
- 구현이나 PR을 준비하면서, dev baseline·run artifact 계약·가벼운 규칙 후보를 다시 확인하려고 본다.
...
| 항목 | 도구 | 깔아두는 이유 |
| --- | --- | --- |
| 의존성 핀 + lock | `pyproject.toml` + uv (또는 poetry, pip-tools) | 재현성. 정책 비교 외부의 변동(라이브러리 버전)을 줄인다 |
| python 버전 핀 | `pyproject.toml`의 `requires-python` | 환경 차이로 인한 결과 변동을 줄인다 |
...
- `sessions/`는 Playground의 가벼운 로그(선택). 외부 인용 출처로 쓰지 않는다.
...
| 새 정책 버전 추가 | 기존 evaluator를 그대로 통과하는 형태를 기준으로 둔다 |
| threshold 변경 | 상수/config 이름으로 바꾸고 `run_meta`에 남긴다 |
| Playground/Test Bench 변경 | 양쪽이 여전히 같은 runner를 호출하는지 확인한다 |
| README/report 수치 작성 | result artifact와 decision log를 먼저 확인한다 (Playground 화면 수치는 인용 출처로 쓰지 않는다) |
| 리팩토링 | 정책 동작 변경과 파일 이동을 가능한 한 분리한다 |

### 검토 역할 (report-only)

작업 흐름의 4지점에서 아래 역할을 참고해 가벼운 검토를 둘 수 있다. 저장소에 같은 역할의 report-only agent가 있으면 활용할 수 있지만, 파일 수정과 최종 판단은 메인 작업자가 한다.
...
- **하네스 invariant 테스트** — 기능 테스트와 별개로 "result overwrite 방지", "snapshot key 존재", "override 복원", "eval-runner 실제 호출", "Playground app도 runner 호출", "criteria version 분리" 같은 하네스 규칙 자체를 테스트.
...
문서 규칙, 체크리스트, 작은 helper는 작업을 빠르게 만들면 먼저 추가해도 된다. 자동 체크(hook/scanner)는 작업 흐름에 영향을 줄 수 있으니 목적과 적용 범위만 짧게 남긴다 (0단계 dev baseline은 바로 적용).

| 반복되는 문제 | 추가할 Sensor 후보 |
| --- | --- |
| result가 덮어써져 이전 비교를 잃음 | run_id 생성 규칙, overwrite 방지 checker |
| threshold가 여러 파일에 흩어짐 | config/named constant check |
| README 수치가 result와 불일치 | evidence check script 또는 report checker |
| Playground 화면 수치가 외부에 인용됨 | 인용 위치에 `run_id` 표기 |
...
핵심은 "처음부터 다 장착하기"가 아니라, 실제로 헷갈린 곳을 다음부터 덜 헷갈리게 만드는 것이다.
...
| 검증된 수치만 공유한다 | 외부 인용은 Test Bench artifact에서 가져오고, Playground 화면 수치는 인용 출처로 쓰지 않는다 |
| 자동 체크는 목적과 범위를 남긴다 | hook/scanner는 적용 대상과 이유를 짧게 적는다 |
...
9. README/report에는 result에서 확인한 수치만 옮긴다 (Playground 화면 수치는 인용 출처로 쓰지 않는다).
