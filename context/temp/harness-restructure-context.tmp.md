...
- `context/archive/product-planning/`: 하네스 정리 이전에 작성된 PRD, 계획서, 용어 결정 노트
...
| 영역 | 의도 |
| --- | --- |
| `context/internal/` | 기획 보강, 용어, 정책 초안, 평가 기준, 설계 메모처럼 팀 안에서 먼저 맞출 내부 기준 자료 |
| `context/decisions/` | 결정, 고민, AI 대화 맥락을 사안별 폴더로 남기는 기록 |
| `context/archive/` | 현재 기준에서 물러난 자료를 보관하는 archive |
| `context/temp/` | 잠깐 공유하거나 정리 전 상태로 둘 임시 자료 |

중요한 흐름은 아래와 같다.

```text
context/archive/product-planning/ 또는 대화/실험에서 나온 재료
-> context/internal/에서 현재 기준으로 정리
-> 기준 변경이면 context/decisions/에 맥락 기록
-> 외부 공유가 필요하면 docs/에 발췌
-> 더 이상 기준이 아니면 context/archive/로 이동
```
...
그래서 이번 PR은 `docs/` 본문을 건드리지 않는다. 대신 `context/internal/`, `context/decisions/`, rule, agent 경계를 먼저 잡고, follow-up에서 공개 문서를 현재 기준에 맞춰 정리한다.

## 왜 기획/을 바로 삭제하지 않았나

`기획/`은 현재 기준 문서가 아니라 하네스 이전 배경 자료다. 다만 그 안에는 용어 결정과 제품 방향의 흔적이 남아 있어 바로 삭제하면 맥락을 잃을 수 있다.

이번 PR에서는 `기획/`을 freeze된 참고 자료로 두고, 새 결정과 기준은 `context/internal/`과 `context/decisions/`로 흐르게 했다. 자료가 충분히 옮겨진 뒤 삭제 여부를 follow-up에서 판단한다.

후속 정리에서 `기획/` 원문은 `context/archive/product-planning/`으로 이동했고, 제품 방향의 active 기준은 `context/internal/product-context.md`로 둔다.
...
- `experiment-material-collector`는 자료를 모을 때 `context/internal/`, `context/decisions/`, `data/`, `results/`를 우선 보도록 했다.
- `harness-structure-reviewer`는 `context/internal/`, `context/decisions/`, `context/archive/`가 active rule이나 코드와 충돌하는지 보도록 했다.
- `result-evidence-checker`는 수치는 `results/runs/{run_id}/`, 어휘는 `context/decisions/`와 active rules 기준으로 확인하도록 했다.
...
- follow-up 문서 정합성 작업이 `context/internal/`과 `context/decisions/`를 기준으로 진행될 수 있다.
...
- 완료: `docs/archive/` 자료를 `context/archive/`로 이전
- 완료: `기획/` 원문을 `context/archive/product-planning/`으로 이동
