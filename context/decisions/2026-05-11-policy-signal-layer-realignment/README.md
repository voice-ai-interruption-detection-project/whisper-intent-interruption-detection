...
핵심은 LLM 사용 여부가 아니라, LLM을 최종 action judge, signal interpreter, ambiguous fallback, evaluator/debugger 중 어느 층위에 둘지 정렬하는 것이다.
...
현재 작업 메모는 `context/temp/analysis-notes/pipeline-layer-realignment-working-context.tmp.md`와 `context/temp/pair-briefs/pipeline-layer-realignment-pair-brief.tmp.md`에 있다.
...
- 나빠진 것: Signal Analyzer, Action Policy, fallback, guard 같은 층을 나누면 구조가 조금 복잡해질 수 있다.
...
- [ ] 코드 변경 전 `Signal Analyzer` / `Action Policy` / `LLM fallback` 경계를 설계한다.
- [ ] run artifact에 signal source, confidence, llm role 같은 필드를 남길 필요가 있는지 검토한다.
