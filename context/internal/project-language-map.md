...
세션 시작 문서는 아니다. 제품 의도를 확인할 때는 [Product Context](product-context.md)를 먼저 보고, 이 문서는 판단 케이스(`scenario`), 입력 경로(`input_mode`), `event_type`, `expected_actions`, `actual_action` 같은 용어가 헷갈릴 때 꺼내 읽는다.
...
| `expected_actions` | action label 중 사람이 정한 자연스러운 AI 행동 기준 | policy가 실제로 낸 결과 |
...
`expected_actions`과 `actual_action`은 서로 다른 종류의 action을 뜻하지 않는다. `expected_actions`는 `continue`, `respond_and_continue`, `stop_and_switch` 같은 action label vocabulary의 목록이고, `actual_action`은 그중 policy가 고른 단일 값이다.
...
| `expected_actions` | 사람이 미리 붙인 정답/기준 label 목록 | action label vocabulary list | `data/scenarios.json` |
| `actual_action` | policy 실행 후 나온 예측/결과 label | action label vocabulary | `results/runs/{run_id}/decision_logs.jsonl` |

예를 들어 `expected_actions = ["stop_and_switch"]`인데 `actual_action = respond_and_continue`라면, 두 값은 같은 action label 집합에 속한다. 다만 기준은 전환이어야 한다고 말하고, policy는 답하고 이어가겠다고 판단했으므로 evaluation에서 불일치로 본다.

## Schema와 Pipeline은 다르다

schema는 판단 케이스 한 장에 필요한 정보를 담는 key/value 목록이다. pipeline은 그 값을 어떤 순서로 처리하는지 나타내는 실행 흐름이다.

예를 들어 `data/scenarios.json`에서 `ai_utterance`, `user_utterance`, `event_type`, `expected_actions`이 나란히 보인다고 해서 모두 같은 층위라는 뜻은 아니다.
...
| `expected_actions` | 사람이 정한 자연스러운 AI 행동 |
...
| `expected_actions` | `actual_action` | action evaluation |
...
이 단위가 작아야 `expected_actions`과 `actual_action`을 비교할 수 있고, 실패 케이스를 다시 분류할 수 있다.
...
| event_type | 고객 신호 | 기본 expected_actions |
| --- | --- | --- |
...
같은 event type이라도 판단 케이스 metadata 수준의 tone/severity hint나 문맥에 따라 expected_actions이 달라질 수 있다. 이때는 `notes`에 판단 근거를 남긴다.
...
| Policy v1 | `policy_v1` | action label 설명과 예시를 더한 현재 비교 대상. tone hint는 실제 음성 테스트 신호로 승격할 때 별도 policy에서 본다 |
...
- `expected_actions`과 `actual_action`을 서로 다른 label 체계처럼 설명하지 않았는가?
