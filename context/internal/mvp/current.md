...
| 영역 | 현재 상태 | MVP에서의 의미 |
| --- | --- | --- |
| `data/scenarios.json` | 커머스 판단 케이스(`scenario`) 30개 있음 | 텍스트 입력(Text Replay)과 Test Bench의 공식 기준 원본 |
| `data/datasets.json` | Test Bench dataset registry | `official`/`diagnostic` dataset을 UI/API에서 선택하게 한다 |
| `data/scenarios_policy_v2_edge.json` | `policy_v2` 진단용 edge slice | backchannel/noise/no_speech false_stop과 짧은 명확 요청 회귀를 따로 본다 |
...
| `results/runs/` | Step 2 hardcoded placeholder 기준 run artifact가 있음 | LLM 전환 또는 공통 파이프라인 적용 후 수치는 새 run artifact를 만들어 다시 인용해야 한다 |
...
| Policy v1 | action label 정의와 예시를 더해 같은 흐름에서 판단 경계가 나아지는지 본다 |
...
| fine-tuning | 현재 MVP는 모델을 새로 학습시키는 단계가 아니라, 고객 신호를 어떤 AI 행동으로 연결할지 검증하는 단계다 |
...
| 표시 라벨 | 코드 식별자 | MVP에서의 읽는 법 |
| --- | --- | --- |
| Baseline | `baseline` | 최소 입력만 보고 판단하는 기준선 |
| Policy v1 | `policy_v1` | action label 설명과 예시를 더한 현재 비교 대상. tone hint는 실제 음성 테스트 신호로 다루는 후속 policy에서 본다 |
| Policy v2 | `policy_v2` | 짧은 맞장구와 비의미 음성에서 false_stop이 줄어드는지 볼 다음 후보 |
| Policy v3 | `policy_v3` | same intent follow-up과 intent shift 구분을 강화할 때의 후보 |
...
9. 공통 고객 신호 해석(`Interpreter Pipeline`)과 AI 행동 선택(`AI Action Selector`)을 추가하고, `baseline`과 `policy_v1`도 같은 흐름을 통과하게 한다.
