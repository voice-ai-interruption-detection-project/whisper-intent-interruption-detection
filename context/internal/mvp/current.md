...
현재 MVP는 전체 상담 AI 앱 구현보다 interruption / intent shift 판단 구조 검증을 우선한다.

Text Replay와 대표 Audio File Test는 제품 개념이 아니라 이 판단 구조를 빠르게 확인하기 위한 현재 입력 경로다.
...
| Text Replay | scenario를 선택하거나 텍스트 입력을 넣어 policy 판단을 먼저 확인한다 |
...
Step별 완료/예정 작업과 과거 혼선 방지 기준은 [Current MVP Iteration Plan](current-iteration-plan.md)을 본다. 아래 순서는 현재 MVP를 닫기 위한 구현 흐름이며, 완료 여부 표시는 아니다.
...
- 대표 Audio File Test가 mock/precomputed transcript라도 같은 AI 행동 판단 입력으로 들어온다.
