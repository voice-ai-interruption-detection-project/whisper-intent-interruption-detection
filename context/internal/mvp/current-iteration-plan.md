...
- 현재 `signals.interpreted_user_intent`, `signals.is_intent_shift`, `signals.confidence`는 LLM이 `actual_action`을 고르면서 함께 남긴 참고값이다. 다음 Step에서는 고객 발화를 어떻게 이해했는지 더 쉽게 확인할 수 있도록, 이 값을 `predicted_event_type`, `predicted_user_intent`, `ambiguity`, `signal_source` 같은 이름으로 정리할지 검토한다.
...
| 후보 | 열리는 조건 |
| --- | --- |
| 공통 고객 신호 해석(`Interpreter Pipeline`) + AI 행동 선택(`AI Action Selector`) | Text/Audio와 `baseline`, `policy_v1`이 같은 해석/행동 흐름을 타야 할 때 |
| Policy v2 (`policy_v2`) | 공통 해석/행동 선택 흐름 위에서 intent shift의 missed switch가 다음 병목으로 확인될 때 |
