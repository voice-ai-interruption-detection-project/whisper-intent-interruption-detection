...
- `policy_v1`은 action label 정의와 few-shot 예시를 더한 LLM 정책이다. scenario metadata tone hint는 실제 음성 신호가 아니므로 `policy_v1` prompt에서는 제외한다.
...
- 현재 구현은 LLM structured output을 사용해 고객 신호 해석 결과만 받고, `RuleBasedActionSelector`가 해석 결과를 `actual_action`으로 매핑한다.
