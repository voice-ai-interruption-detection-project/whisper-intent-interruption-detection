...
- 기존 `actual_action` 생성과 `expected_action` vs `actual_action` 평가 흐름은 유지한다.
...
- 기존에도 LLM user prompt에는 `event_type`, `expected_user_intent`, `expected_action`을 넣지 않았다. 이번 구현은 그 guard를 유지하면서, runner가 policy 호출 시 runtime 필드만 담은 `PolicyInput`으로 변환해 policy 입력 표면을 좁힌다.
...
- `event_type`, `expected_user_intent`, `expected_action`은 evaluator/error analysis 기준값으로만 쓰며, LLM user prompt에는 넣지 않는다.
