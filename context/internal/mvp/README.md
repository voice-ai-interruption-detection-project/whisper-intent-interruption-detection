...
이 폴더의 기준은 "지금 active인 MVP"다. 과거 `context/archive/product-planning/`의 초기 MVP 기획 문서는 방향을 만든 evidence로만 보고, 여기서는 현재 action label, scenario schema, run artifact 계약에 맞춘 MVP 범위와 Day별 작업 흐름만 정리한다.
...
| 과거 기획 원문 | [Archived Product Planning](../../archive/product-planning/README.md) |
...
- [Current MVP Iteration Plan](current-iteration-plan.md)은 그 기준을 Day별 작업 흐름으로 풀어 둔다.
- Day 1, Day 2 같은 작업 순서를 현재 기준으로 유지한다.
- 완료된 작업은 commit, run artifact, 현재 repo 상태와 함께 적는다.
- 예정 작업은 planned로만 적고, 완료처럼 쓰지 않는다.
- 기존 기획 자료에서 유용한 진행감은 가져오되, 낡은 용어와 수치는 되살리지 않는다.
...
| `planned` | 다음 Day/Week 후보. 아직 완료나 확정 기준이 아님 | `current-iteration-plan.md`의 planned/candidate 영역 |
| `candidate` | 새 MVP나 2주차 MVP 후보. active 기준과 섞이면 안 됨 | 필요하면 `context/temp/` 또는 새 decision 초안 |
| `evidence` | 왜 이 방향이 나왔는지 보는 과거 자료 | `context/archive/`, `context/decisions/` |
...
| 헷갈리기 쉬운 것 | 현재 기준 |
| --- | --- |
| Day N | 달력 날짜가 아니라 MVP 작업 iteration이다. 완료된 Day는 실제 날짜와 commit을 함께 적는다 |
| 초기 MVP 문서 | active 기준이 아니라 방향을 만든 archive evidence다 |
...
| 다른 브랜치의 scaffold/run artifact | current MVP의 입력으로 보지 않는다. 현재 브랜치 기준으로 새로 구현하고 새 run을 만든다 |
| Workbench | 완성형 상담 앱이 아니라 Playground와 Test Bench를 묶는 실험 콘솔 후보 이름이다 |
...
- Day별 계획이 아니라 MVP의 완료 기준 자체가 바뀐다.
...
5. [Archived Product Planning](../../archive/product-planning/README.md), 단 history/evidence로만
