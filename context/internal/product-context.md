...
| 결정 이유와 대안 | [context/decisions](../decisions/) |
...
| AI Action Policy 비교 | transcript와 speech signal을 입력으로 쓰는 LLM 판단 정책을 비교해 어떤 실패가 줄어드는지 본다 |
...
아래는 실제 변경이 일어나면 `context/decisions/`에 사안별로 기록한다.
...
초기 기획 원문은 [context/archive/product-planning](../archive/product-planning/)에 보관한다. 아래 내용은 active 기준으로 쓰지 않고, 필요하면 archive의 history/evidence로만 본다.
