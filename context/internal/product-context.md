...
텍스트 입력(Text Replay), 오디오 파일 입력(Audio File Test), 마이크 입력 후보(Mic Trial)는 제품의 본질적 개념이 아니라 같은 판단 구조를 어떤 입력으로 실행해 볼지 나타내는 개발/테스트 경로다.
...
- 마이크 입력 후보(Mic Trial): live 입력, latency, browser permission 리스크를 나중에 확인하는 확장 슬롯이다.
...
| 배치 평가 화면(`Test Bench`) | 여러 판단 케이스를 batch로 돌려 수치를 남기는 평가 표면 |
