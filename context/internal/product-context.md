...
현재 `baseline`과 `policy_v1`은 transcript와 AI context를 보고 LLM이 바로 `actual_action`을 고르는 direct action judge다.
...
따라서 다음 구조 후보는 Text/Audio 입력 뒤에 공통 고객 신호 해석(`Interpreter Pipeline`)과 AI 행동 선택(`AI Action Selector`)을 세우고, `baseline`과 `policy_v1`도 같은 흐름을 통과하게 하는 것이다. policy version은 이 공통 흐름 위에서 어떤 신호와 기준을 더 쓰는지 비교하는 단위로 남긴다.
...
- 현재 LLM이 바로 `actual_action`을 고를 때 쓰는 prompt에 `expected_action`, `event_type`, `expected_user_intent`가 들어가지 않는다.
