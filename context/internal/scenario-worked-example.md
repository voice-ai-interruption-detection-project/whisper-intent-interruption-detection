...
`event_type`, `expected_user_intent`, `expected_action`은 사람이 붙인 기준값이다. 현재 LLM structured output prompt에는 `expected_action`, `event_type`, `expected_user_intent`를 넣지 않는다.
...
코드에서는 기존처럼 LLM user prompt에 `event_type`, `expected_user_intent`, `expected_action`을 넣지 않는다. `RunnerInput`이 API/evaluator 경계의 기준값을 보관하더라도, runner는 policy 호출 시 runtime 필드만 담은 `PolicyInput`으로 변환한다.
