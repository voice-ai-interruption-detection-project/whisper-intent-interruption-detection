...
이 문서는 한 판단 케이스(`scenario`)가 상황 요약, schema key/value, policy 판단, 평가 결과로 어떻게 이어지는지 보여주는 내부 기준 예시다.
...
-> expected_actions / actual_action 비교
-> failure taxonomy
```

schema key의 나열 순서가 곧 실행 순서는 아니다. schema는 한 판단 케이스 카드(scenario card)에 필요한 정보를 담는 그릇이고, pipeline은 그 정보를 어떤 순서로 처리하는지 나타낸다.
...
| `event_type` | `intent_shift` | 사람이 붙인 기준 고객 신호 |
| `expected_user_intent` | `refund_request` | 사람이 붙인 기준 고객 의도 |
| `expected_actions` | `["stop_and_switch"]` | 사람이 정한 자연스러운 AI 행동 목록 |

`event_type`은 고객 신호이고, `expected_actions`은 AI 행동이다. 이 둘은 같은 층위가 아니다.

### Expected와 Actual의 역할

판단 케이스 카드에는 `expected_actions`만 있다. `actual_action`은 policy를 실행한 뒤에 생긴다.

두 값은 서로 다른 label 체계가 아니라 같은 action label vocabulary를 다른 역할로 쓰는 것이다.

| 이름 | 이 예시의 값 | 역할 |
| --- | --- | --- |
| `expected_actions` | `["stop_and_switch"]` | 사람이 미리 정한 기준/정답 |
...
현재 구현 전의 direct LLM action judge 흐름은 아래에 가까웠다.

```text
텍스트 입력
-> user_utterance를 고객 transcript처럼 사용
-> ai_current_intent / ai_utterance / transcript / speech signal로 PolicyInput 구성
-> LLM이 고객 신호 해석과 action 선택을 한 번에 처리
-> actual_action 생성
-> expected_actions과 actual_action 비교
```

오디오 파일 입력이나 마이크 입력(Mic Trial)으로 넘어가도 뒤쪽 판단 구조는 같다. 달라지는 것은 `user_utterance`가 바로 들어오느냐, 오디오가 STT/signal 단계를 거쳐 transcript와 speech signal로 바뀌느냐다. `event_type`과 `expected_user_intent`는 policy 입력이 아니라 evaluation과 error analysis에서 케이스 의미를 해석하는 기준값으로 본다.

코드에서는 기존처럼 LLM user prompt에 `event_type`, `expected_user_intent`, `expected_actions`을 넣지 않는다. `RunnerInput`이 API/evaluator 경계의 기준값을 보관하더라도, runner는 policy 호출 시 runtime 필드만 담은 `PolicyInput`으로 변환한다.

현재 구조는 아래처럼 해석 결과와 action 선택을 나눠 기록한다.

```text
transcript / speech signal
-> 고객 신호 해석(`Interpreter Pipeline`)
   predicted_event_type = intent_shift
   predicted_user_intent = refund_request
   confidence = 0.91
-> AI 행동 선택(`AI Action Selector`)
   actual_action = stop_and_switch
-> expected_actions과 actual_action 비교
-> event_type과 predicted_event_type은 고객 신호 해석 점검으로 따로 비교
```
...
| actual_action | 해석 |
| --- | --- |
| `stop_and_switch` | expected_actions에 포함되어 일치 |
...
| actual_action | 해석 |
| --- | --- |
| `continue` | expected_actions에 포함되어 일치 |
| `brief_ack` | expected_actions에 포함되어 일치 |
...
| `predicted_event_type`은 `event_type`을 새 이름으로 바꾼 것이다 | 아니다. `event_type`은 사람이 붙인 기준값이고, `predicted_event_type`은 runtime 해석 결과 후보이다 |
| `expected_actions`은 policy 실행 결과다 | 아니다. 사람이 정한 기준 행동이다 |
| `expected_actions`과 `actual_action`은 서로 다른 action 체계다 | 아니다. 같은 action label vocabulary를 기준/결과 역할로 나눈 것이다 |
| `actual_action`을 `data/scenarios.json`에 넣어도 된다 | 아니다. 실행 결과는 `results/runs/{run_id}/`에 둔다 |
| 텍스트 입력(Text Replay)은 음성 프로젝트에서 벗어난다 | 아니다. audio/STT 앞단을 고정하고 policy 판단만 먼저 검증하는 입력 경로다 |
| `scenario`가 전체 상담 플로우다 | 아니다. policy 평가 가능한 한 순간의 판단 케이스다 |
...
3. 어떤 값이 policy input이고 어떤 값이 사람이 정한 기준값/정답인지 나눈다.
4. `expected_actions`과 `actual_action`을 비교한다.
