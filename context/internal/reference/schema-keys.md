...
`event_type`은 값을 담는 자리이고, `intent_shift`는 그 자리에 들어가는 값이다. `expected_actions = ["stop_and_switch"]`도 마찬가지다. `expected_actions`는 기준 행동 목록을 담는 key이고, `stop_and_switch`는 action label vocabulary의 값이다.
...
| `action_match` | `true`, `false` | `actual_action`이 `expected_actions`에 포함되는지 여부 | 평가 후 | `decision_logs.jsonl` |
| `action_accuracy` | `0.83` | 전체 action match 비율 | 평가 후 | `evaluation.json` |
...
| `expected_actions = ["respond_and_continue"]` | 사람이 정한 기준 행동 목록이 답하고 이어가기만 허용한다는 뜻 | policy가 이미 그렇게 실행했다는 뜻이 아니다 |
...
- `expected_actions`를 policy 실행 결과처럼 설명하지 않았는가?
