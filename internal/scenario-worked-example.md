...
이 문서는 한 scenario가 제품 장면, schema key/value, policy 판단, 평가 결과로 어떻게 이어지는지 보여주는 내부 기준 예시다.
...
-> event_type / expected_user_intent를 기준으로 policy input 구성
-> AI Action Policy 실행
-> actual_action 생성
-> expected_action과 actual_action 비교
```

Audio File Test나 Mic Trial로 넘어가도 뒤쪽 구조는 같다. 달라지는 것은 `user_utterance`가 바로 들어오느냐, 오디오가 STT/signal 단계를 거쳐 transcript와 intent 후보로 바뀌느냐다.
...
| 헷갈리는 생각 | 정정 |
| --- | --- |
| `input_mode`는 AI가 말하는 방식이다 | 아니다. 같은 scenario를 Text, Audio, Mic 중 어떤 방식으로 넣는지다 |
...
| Text Replay는 음성 프로젝트에서 벗어난다 | 아니다. audio/STT 앞단을 고정하고 policy 판단만 먼저 검증하는 단계다 |
...
3. 어떤 값이 policy input이고 어떤 값이 기준 정답인지 나눈다.
