...
구체 예시는 [Scenario Worked Example](scenario-worked-example.md)을 먼저 본다. 특히 `input_mode`, `event_type`, `expected_action`이 한 화면에 같이 나와 헷갈릴 때는 예시를 따라가면 된다.
...
-> Event Type Reference / Action Label Reference
...
| 문서 | 볼 때 |
| --- | --- |
| [Schema Key Reference](schema-key-reference.md) | `event_type = intent_shift`에서 key와 value가 헷갈릴 때 |
| [Event Type Reference](event-type-reference.md) | `backchannel`, `intent_shift`, `ambiguous` 같은 고객 신호 값을 확인할 때 |
| [Action Label Reference](action-label-reference.md) | `continue`, `respond_and_continue`, `stop_and_switch` 같은 AI 행동 값을 확인할 때 |
...
key별 세부 설명은 [Schema Key Reference](schema-key-reference.md)를 본다.
...
값별 세부 설명은 [Event Type Reference](event-type-reference.md)를 본다.
...
값별 세부 설명은 [Action Label Reference](action-label-reference.md)를 본다.
...
- `input_mode`를 AI 발화 방식처럼 설명하지 않았는가?
