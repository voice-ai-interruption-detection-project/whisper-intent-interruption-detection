...
- `archive/product-planning/`: 하네스 정리 이전에 작성된 PRD, 계획서, 용어 결정 노트
...
```text
archive/product-planning/ 또는 대화/실험에서 나온 재료
...
후속 정리에서 `기획/` 원문은 `archive/product-planning/`으로 이동했고, 제품 방향의 active 기준은 `internal/product-context.md`로 둔다.
...
- 완료: `기획/` 원문을 `archive/product-planning/`으로 이동
