...
- `TO_DO:` placeholder, 미구현 endpoint, 임시 mock 반환이 남아 있을 때
...
작업 안에 `TO_DO:` placeholder가 있으면 어느 파일에 남겼는지, 지금 왜 필요한지, 다음 구현 단위를 남은 위험 또는 후속 작업에 적는다.
...
- commit 제목의 type/scope, 코드 식별자, action label, metric, API 필드, 명령어는 영어 그대로 둔다.
- 그 밖의 설명 문장은 한국어를 기본으로 쓴다. 영어 단어 사용을 막는 규칙이 아니라, PR/작업 메모/가이드 본문이 통째로 전문 영어 톤으로 바뀌는 것을 막기 위한 기준이다.
- 영어 원문을 인용하거나 외부 API 에러를 그대로 옮길 때는 필요한 만큼만 남기고, 판단·이유·후속 작업은 한국어로 쓴다.
