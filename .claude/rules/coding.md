...
현재 코드 표면은 Playground와 배치 평가(Test Bench) 두 surface가 같은 runner로 정책 버전을 갈아끼워 결과를 비교하는 실험 작업대 형태다. 그래서 **변경의 단위가 곧 비교의 단위**가 된다. 한 PR 안에서 동작 변경과 파일 이동이 섞이면 "왜 결과가 바뀌었는지"가 흐려진다. 아래 항목은 그 비교 가능성이 깨지지 않게 하는 작은 약속들이다.
