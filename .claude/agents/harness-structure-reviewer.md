...
- `data/`는 read-only ground truth (시나리오, expected_action, audio mapping). 여기에 `actual_action`이 들어가면 안 된다.
...
4. evaluator가 runner를 호출하지 않고 policy 판정 로직을 다시 구현하는가.
