...
description: 코드 변경 후 하네스 경계(input adapter / policy / evaluator / runner / results)가 새지 않았는지 점검한다. 새 policy 추가, runner 리팩토링, 새 input mode 추가, 구조 변경 commit 직전에 사용한다. internal/의 기획 보강·용어·정책·평가 기준 자료, decisions/와 active rule 동기화, archive/ 격리 여부까지 boundary로 본다. 발견된 boundary leak을 file:line 인용과 함께 반환한다. report-only.
...
- `internal/`은 기획 보강, 용어 정리, 정책 초안, 평가 기준, 설계 메모 같은 내부 기준 자료를 맞추는 작업 자료층이다. 정합된 기준은 필요한 active 위치로 반영되어야 한다.
