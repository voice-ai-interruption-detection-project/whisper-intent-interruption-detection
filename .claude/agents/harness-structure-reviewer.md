...
description: 코드 변경 후 하네스 경계(input adapter / policy / evaluator / runner / results)가 새지 않았는지 점검한다. 새 policy 추가, runner 리팩토링, 새 input mode 추가, 구조 변경 commit 직전에 사용한다. 발견된 boundary leak을 file:line 인용과 함께 반환한다. report-only.
...
- `sessions/`(있을 경우)는 Playground의 가벼운 로그. 외부 인용 출처가 될 수 없다.

# 점검 항목

prompt에서 지정한 변경 범위 또는 diff에 대해 다음 9개를 본다.
...
4. evaluator나 Playground 진입(`src/app.py`)이 runner를 호출하지 않고 policy 판정 로직을 다시 구현하는가.
...
9. Playground 화면 결과가 `results/runs/{run_id}/` 형식으로 저장되거나, 반대로 Test Bench artifact가 `sessions/`에 새지 않는가 (두 surface 산출물 경계).
