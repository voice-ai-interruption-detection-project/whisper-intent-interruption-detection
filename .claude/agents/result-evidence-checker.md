...
description: README, report, 공유 문서의 수치가 `results/runs/{run_id}/` 아티팩트와 일치하는지 검증한다. 결과 발표 직전, presentation 공유 직전, report PR 머지 직전에 사용한다. claim 단위로 supported / unsupported / mismatch / not-found / ambiguous 판정과 출처 경로를 반환한다. report-only.
