...
| `internal/`          | 내부 기준 자료를 맞추는 작업 자료층 | 기획 보강, 용어 정리, 정책 초안, 평가 기준, 설계 메모를 현재 어휘로 정리한다.                      |
...
| `docs/archive/`      | 기존 docs 내부 archive         | 이전 자료가 남아 있는 전환 영역이다. 명시 요청 없이 작업 가드, 인용 출처, 구현 근거로 쓰지 않는다. |
| `temp/`              | 임시 작업·짧은 공유 자료      | `.tmp.md` suffix를 권장한다. 오래 남으면 `internal/`이나 `decisions/`로 승격할지 본다.              |
| `기획/`              | freeze된 과거 배경 자료        | 신규 결정·기준 자료는 추가하지 않는다. 명시 요청 시에만 참조하고, 사용 후 삭제 여부를 검토한다.    |
...
- 현재 기준에서 물러난 자료는 `archive/`로 보낸다. 기존 `docs/archive/`는 follow-up에서 이전할 수 있는 전환 영역이다.
- `기획/`은 freeze된 배경 자료다. 신규 결정·기준 자료는 `decisions/`·`internal/`로 흐른다.
