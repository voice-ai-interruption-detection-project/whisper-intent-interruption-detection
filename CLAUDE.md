...
- `docs/`는 참고용 자료 archive다. 작업 가드나 인용 출처로 쓰지 않고, 다른 문서·규칙·agent 정의에서 링크하지 않는다.
- `.claude/rules/`는 **작업할 때 참고하는 가드 layer**다. 기본 출발점은 실험 측 `experiments.md`, 개발 측 `coding.md`, 작업 운영 측 `workflow.md`이고, 새 작업 영역이 생기면 필요한 만큼 작게 나눈다.
- 실험 프로토콜, agent 후보, hook/scanner 같은 세부 항목은 개발을 편하게 만들 만큼 구체화되면 작은 문서·체크리스트·agent 정의로 둔다.
