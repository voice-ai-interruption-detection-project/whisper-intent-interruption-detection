...
현재 MVP는 전체 음성 상담 서비스를 완성하기보다, AI가 말하는 중 들어온 고객 발화를 보고 다음 행동을 고르는 판단 구조를 Text Replay와 대표 Audio File Test 같은 입력 경로로 검증한다. Whisper/STT/audio adapter는 이 판단 구조를 음성 입력 쪽으로 연결하기 위한 구현 요소다.
