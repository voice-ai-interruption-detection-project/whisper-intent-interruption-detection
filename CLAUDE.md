...
- `docs/`는 mkdocs로 외부 배포되는 공유/공개 문서 영역이다.
- `internal/`은 내부 기준 자료를 맞추는 작업 자료층이다. 기획 보강, 용어 정리, 정책 초안, 평가 기준, 설계 메모는 정합된 어휘(`respond_and_continue`, primary 5종, `results/runs/{run_id}/`)로 여기서 정리하고, 안정되면 필요한 active 위치로 반영한다.
- `decisions/`는 결정·고민·AI 대화 맥락의 단일 보관소다. 라벨·정책·평가 기준이 바뀌거나 탐색 중일 때 `record-decision` skill로 사안별 폴더를 추가한다.
- `archive/`, `temp/`는 공개 문서 영역이 아니다. `temp/`는 `.tmp.md` suffix를 권장하며, 필요하면 승격/삭제를 검토한다.
- `docs/archive/`는 과거 조사·설계 후보를 보관하는 영역이다. AI 작업자는 사용자가 명시적으로 요청한 경우를 제외하고 archive 문서를 작업 가드, 인용 출처, 구현 근거로 직접 참조하거나 활용하지 않는다.
- `기획/`은 freeze된 작업 배경 자료(PRD·계획·방향성 메모)를 담는 내부 영역이다. 신규 결정·기준 자료는 추가하지 않고 `decisions/`·`internal/`로 흐른다. AI 작업자는 사용자가 명시적으로 요청한 경우에만 참조하고, 외부 노출용 mkdocs 사이트(`docs/`)에는 포함하지 않는다. 사용이 끝나면 follow-up에서 삭제 여부를 검토한다.
