...
Backend 의존성(FastAPI + Whisper STT + sentence-transformers)과 dev baseline(Poetry lock, ruff, pre-commit + detect-secrets)이 들어와 있다. Day 3 기준으로 scenario loader, 공통 타입, 텍스트 LLM 기반 `baseline`/`policy_v1`, runner CLI, evaluator artifact, FastAPI API, 정적 Playground UI, Audio File Test adapter가 동작한다.
