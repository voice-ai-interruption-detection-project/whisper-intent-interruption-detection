...
Backend 의존성(FastAPI + Whisper STT + sentence-transformers)과 dev baseline(Poetry lock, ruff, pre-commit + detect-secrets)이 들어와 있다. Day 3 기준으로 scenario loader, 공통 타입, 텍스트 LLM 기반 `baseline`/`policy_v1`, runner CLI, evaluator artifact, FastAPI API, 정적 Playground UI가 동작한다.
...
# 6. Backend API + Playground 실행
poetry run uvicorn backend.main:app --reload
```

Playground는 `http://127.0.0.1:8000`에서 열린다. `OPENAI_API_KEY`가 없으면 실제 LLM policy 실행은 실패한다. 테스트는 fake LLM client로 네트워크 없이 검증한다. 공식 수치는 화면 표시가 아니라 `results/runs/{run_id}/evaluation.json`을 기준으로 인용한다.
